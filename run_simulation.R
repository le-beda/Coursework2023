...
#' @param  plateau acceptance threshold 
#' 
...
#' @examples
run_simulation<-
  function(simulator, epochs, calculate.pcf=FALSE, pcf_grid, auto_stop_at_plateau = TRUE, plateau_threshold = 45) {
    print("begin.")
    require(dplyr)
    time<-numeric(epochs+1)
    pop<-numeric(epochs+1)
    exp_pop<-rep(list(numeric(epochs+1)), each = 5)
    ...
    simulation_windows_sizes<-c(15, 45)
    last_windows<-c(list(pop[1]), list(pop[1]))
    windows_var<-list(list(), list())
    alpha = 0.1 # константа экспаненциального сглаживания
    found_plateau = FALSE
    print("epochs")
    print(epochs)
